...
Note that vidulumd will need to be stopped and restarted for edits in the config file to take effect. 

### Using `backupwallet`

To create a backup of your wallet, use:

`vidulum-cli backupwallet <nameofbackup>`.
...
`vidulum-cli z_exportwallet <nameofbackup>`
...
`vidulum-cli z_importwallet </path/to/exportdir/nameofbackup>`
...
`vidulum-cli z_exportkey <z-address>`
...
`vidulum-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`vidulum-cli z_importkey <z-priv-key>`
...
`vidulum-cli help importprivkey`
