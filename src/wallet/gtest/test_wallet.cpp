...
CWalletTx GetValidReceive(const libvidulum::SpendingKey& sk, CAmount value, bool randomInputs) {
    return GetValidReceive(*params, sk, value, randomInputs);
}

libvidulum::Note GetNote(const libvidulum::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSpend(const libvidulum::SpendingKey& sk,
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libvidulum::SpendingKey::random();
    auto sk2 = libvidulum::SpendingKey::random();
