...
bool CWallet::AddZKey(const libvidulum::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libvidulum::PaymentAddress &address,
                                    const libvidulum::ReceivingKey &rk,
...
bool CWallet::LoadCryptedZKey(const libvidulum::PaymentAddress &addr, const libvidulum::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, rk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libvidulum::SpendingKey &key)
{
    return CCryptoKeyStore::AddSpendingKey(key);
}

bool CWallet::AddViewingKey(const libvidulum::ViewingKey &vk)
{
...
bool CWallet::RemoveViewingKey(const libvidulum::ViewingKey &vk)
{
...
bool CWallet::LoadViewingKey(const libvidulum::ViewingKey &vk)
{
                                                   ...
                                                   uint8_t n) const
{
    ...
    auto note_pt = libvidulum::NotePlaintext::decrypt(
        dec,
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent, bool ignoreUnspendable)
{
    ...
    libvidulum::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
