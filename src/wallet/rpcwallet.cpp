...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Vidulum address: ")+name_);
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Vidulum address: ")+input.get_str());
            if (setAddress.count(address))
...
UniValue zc_sample_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    JSDescription samplejoinsplit(*pvidulumParams,
                                  pubKeyHash,
                                  anchor,
                                  {JSInput(), JSInput()},
                                  {JSOutput(), JSOutput()},
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
    ...
    JSDescription jsdesc(*pvidulumParams,
                         joinSplitPubKey,
                         anchor,
                         {vjsin[0], vjsin[1]},
                         {vjsout[0], vjsout[1]},
...
UniValue z_getnewaddress(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 0)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "\nResult:\n"
            "\"vidulumaddress\"    (string) The new zaddr\n"
            "\nExamples:\n"
...
UniValue z_listaddresses(const UniValue& params, bool fHelp)
{
    ...
    UniValue ret(UniValue::VARR);
    std::set<libvidulum::PaymentAddress> addresses;
...
UniValue z_listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    ...
    libvidulum::PaymentAddress zaddr;
...
UniValue z_shieldcoinbase(const UniValue& params, bool fHelp)
{
    ...
    try {
        ...
        libvidulum::PaymentAddress zaddr = pa.Get();
    } catch (const std::runtime_error&) {
