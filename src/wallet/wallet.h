...
{
public:
    libvidulum::PaymentAddress address;
    ...
    CNoteData(libvidulum::PaymentAddress a) :
            address {a}, nullifier(), witnessHeight {-1} { }
    CNoteData(libvidulum::PaymentAddress a, uint256 n) :
...
{
    JSOutPoint jsop;
    libvidulum::NotePlaintext plaintext;
};
...
public:
    ...
    std::map<libvidulum::PaymentAddress, CKeyMetadata> mapZKeyMetadata;
    ...
    /// Get15000VDL output and keys which can be used for the Masternode
    ...
    bool AddZKey(const libvidulum::SpendingKey &key);
    //! Adds spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadZKey(const libvidulum::SpendingKey &key);
    //! Load spending key metadata (used by LoadWallet)
    bool LoadZKeyMetadata(const libvidulum::PaymentAddress &addr, const CKeyMetadata &meta);
    //! Adds an encrypted spending key to the store, without saving it to disk (used by LoadWallet)
    bool LoadCryptedZKey(const libvidulum::PaymentAddress &addr, const libvidulum::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);
    //! Adds an encrypted spending key to the store, and saves it to disk (virtual method, declared in crypter.h)
    bool AddCryptedSpendingKey(const libvidulum::PaymentAddress &address, const libvidulum::ReceivingKey &rk, const std::vector<unsigned char> &vchCryptedSecret);

    //! Adds a viewing key to the store, and saves it to disk.
    bool AddViewingKey(const libvidulum::ViewingKey &vk);
    bool RemoveViewingKey(const libvidulum::ViewingKey &vk);
    //! Adds a viewing key to the store, without saving it to disk (used by LoadWallet)
    bool LoadViewingKey(const libvidulum::ViewingKey &dest);
    ...
    boost::optional<uint256> GetNoteNullifier(
        ...
        const libvidulum::PaymentAddress& address,
