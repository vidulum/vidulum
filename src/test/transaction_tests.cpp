...
BOOST_AUTO_TEST_CASE(tx_valid)
{
    ...
    auto verifier = libvidulum::ProofVerifier::Strict();
    ScriptError err;
...
BOOST_AUTO_TEST_CASE(test_basic_joinsplit_verification)
{
    ...
    //
    // Also, it's generally libvidulum's job to ensure the
    ...
    {
        JSDescription jsdesc(*pvidulumParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        BOOST_CHECK(jsdesc.Verify(*pvidulumParams, verifier, pubKeyHash));
        ...
        BOOST_CHECK(jsdesc_deserialized.Verify(*pvidulumParams, verifier, pubKeyHash));
    }

    {
        // Ensure that the balance equation is working.
        BOOST_CHECK_THROW(JSDescription(*pvidulumParams, pubKeyHash, rt, inputs, outputs, 10, 0), std::invalid_argument);
        BOOST_CHECK_THROW(JSDescription(*pvidulumParams, pubKeyHash, rt, inputs, outputs, 0, 10), std::invalid_argument);
    }

    {
        // Ensure that it won't verify if the root is changed.
        auto test = JSDescription(*pvidulumParams, pubKeyHash, rt, inputs, outputs, 0, 0);
        test.anchor = GetRandHash();
        BOOST_CHECK(!test.Verify(*pvidulumParams, verifier, pubKeyHash));
    }
}
